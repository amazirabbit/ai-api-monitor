...
/**
 * 余额缓存 — 仅存各账号最新余额，用于首页快速展示和排序。
 *
 * ⚠️ 不再包含 usedThisMonth / usedToday 等计算字段，
 * 所有消费统计统一由 BalanceSnapshot Diff 实时计算。
 */
...
data class BalanceInfo(
    ...
    fun format(): String {
        ...
        return "$sym${"%.2f".format(totalBalance)}"
    }

    val timeAgo: String get() {
        val ms = System.currentTimeMillis() - updatedAt
        val sec = ms / 1000
        val min = sec / 60
        return when {
            sec < 10 -> "刚刚"
            sec < 60 -> "${sec}秒前"
            min < 60 -> "${min}分钟前"
            min < 1440 -> "${min / 60}小时${min % 60}分钟前"
            else -> "${min / 1440}天前"
        }
    }

    val updateTimeStr: String get() {
...
data class TotalBalance(
    ...
    val formatted: String get() = "¥${"%.2f".format(totalCny)}"
}